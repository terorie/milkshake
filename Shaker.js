var Shaker = Class.extend({
    ...
    if (this.count % 100 == 0) {
      this.renderer.realfps =
        ...
        ((TimeKeeper.getTicks(this.timeKeeper.startTime) - this.fpsstart) /
          1000);
      this.infoMessages["fps"] =
        "rendering at " +
        Math.round(this.renderer.realfps * 100) / 100 +
        " frames per second";
  ...
  selectNext: function(hardCut) {
    ...
    this.presetPos++;
    if (!hardCut) this.activePreset2 = this.switchPreset();
    else {
  ...
  renderInfoBox: function() {
    ...
    if (this.infoBoxPos > -1) {
      ...
      this.infoBox.innerHTML = this.infoMessages[
